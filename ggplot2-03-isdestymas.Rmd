...
ggplot(data = mpg) + 
 ...
 ggplot(data = mpg) +
   geom_point(mapping = aes(x = cty, y = hwy, shape = cty)) 
```
...
size naudojamas su skaitiniais kintamaisiais, o colors ir shape su kategoriniais
...
```{r}
 ggplot(data = mpg) +
   geom_point(mapping = aes(x = manufacturer, y = cty), stroke = 1)
```
reik mokėt help naudotis. stroke yra dydis
...
```
kelios skalės į dvi atskiras legendas
...
```{r}
 ggplot(data = mpg) +
   geom_point(mapping = aes(x = cty, y = hwy, color = cty, shape = cty))
```
į tą pat legendą
